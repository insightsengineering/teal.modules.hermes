...
test_that("tm_g_volcanoplot works as expected in the sample app", {
  ...
  library(shinytest)
  app <- ShinyDriver$new("tests/testthat/volcanoplot/", loadTimeout = 1e5, debug = "all" )
  app$getDebugLog()
  ...
  initial_facet_var <- app$waitForValue("teal-main_ui-modules_ui-root_volcanoplot-facet_var")
  expect_identical(initial_facet_var, "AGE18")
  ...
  # Now change the experiment_name and confirm that facet_var is updated accordingly.
  app$setInputs(
    "teal-main_ui-modules_ui-root_volcanoplot-experiment_name" = "se3"
  )
  now_facet_var <- app$waitForValue("teal-main_ui-modules_ui-root_volcanoplot-facet_var")
  expect_identical(now_facet_var, "AGE18")
