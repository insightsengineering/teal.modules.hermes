...
srv_made_up_merge_pr <- function(input, output, session, datasets, dataname) {
  ...
  # genes <- eventReactive(experiment_data(), ignoreNULL = FALSE, {
...
# 
# @return A merged data frame containing all columns/rows from adtte and select columns from MAE (counts, Sample IDs) for a given gene.
# 
...
{
  anl <- binned_adtte
  ANL <- radtte(cached = TRUE) %>% mutate(CNSR = as.logical(CNSR)) 
    anl <- ANL %>% mutate(ARM = droplevels(ARM)) %>% mutate(is_event = CNSR == 
        0)
}
...
$graph
{
    ...
    result <- mapply(df = as.data.frame(anl), nrow = seq_along(levels(anl$SEX)),
