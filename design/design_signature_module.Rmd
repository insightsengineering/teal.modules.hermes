...
GeneSpec <- R6::R6Class(
  ...
  public = list(
    initialize = function(genes,
      ...
      assert_character(genes, min.chars = 1L, min.len = 1L, any.missing = FALSE, unique = TRUE)
      assert_function(summary_fun, null.ok = TRUE)
      assert_string(summary_name, min.chars = 1L)
      
      private$genes <- genes
      private$summary_fun <- summary_fun
      private$summary_name <- summary_name
      private$vector_valued <- identical(length(genes), 1L) || is.function(summary_fun)
    },
    get_genes = function() {
      private$genes
    },
    returns_vector = function() {
      private$vector_valued
    },
    get_label = function() {
      assert_true(private$vector_valued) # only makes sense if vector valued
      if (length(private$genes) > 1) {
        paste0(private$summary_name, parenthesize(shorten_list(private$genes)))
      } else {
        private$genes  
      }
    },
    extract = function(assay) {
      ...
      if (length(private$genes) > 1 && is.function(private$summary_fun)) {
        summary_res <- private$summary_fun(assay_cols)
